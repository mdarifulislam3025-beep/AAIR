...
import { acceptFlightResponse, createFlightRequest, getFlightRequestDetails, listUserFlightRequests } from "@/modules/flights/flight.service";

function validDateList(dates: unknown): dates is string[] {
  return Array.isArray(dates) && dates.length > 0 && dates.every((d) => !Number.isNaN(Date.parse(String(d))));
}
...
export async function createFareMessage(request: NextRequest): Promise<NextResponse> {
  ...
  const data = createFlightRequest(auth.userId, { tripType: tripType as "OW" | "RT" | "MC", from, to, travelDates, pax, cabinClass: cabinClass as "ECONOMY" | "PREMIUM_ECONOMY" | "BUSINESS" | "FIRST" });
...
export async function getMyFareMessage(request: NextRequest, id: number): Promise<NextResponse> {
  ...
  if (auth instanceof NextResponse) return auth;
  try {
    return ok(getFlightRequestDetails(id, auth.userId, "customer"), "Flight fare request details fetched");
  } catch (e) {
